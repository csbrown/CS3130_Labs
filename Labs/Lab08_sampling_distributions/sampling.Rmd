...
In order to do any simulations, we will need an example population.  We will be using the Poisson distribution.  The specific property of the Poisson distribution that will make it interesting for us is that the [*skewness*](https://en.wikipedia.org/wiki/Skewness) of the Poisson is $\lambda^{-\frac{1}{2}}$.  *Skewness* is a measure of how "asymmetric" the distribution is.  For example, the normal distribution (which is perfectly symmetric about the mean) has skewness 0.  The exponential distribution has skewness 2 (no matter what the paramter $\lambda$ is!).  In the exponential distribution, all of the values to the left of the mean are still pretty close to the mean (recall that the support of the exponential distribution is $(0, \infty)$), but values to the right of the mean may well be quite large.  This is the essense of skewness.
...
You'll need to adjust the limits and also estimate the most likely parameters for your normal distribution from your data.
...
Recall that the mean and variance of the Poisson distribution are both $\lambda$.  Verify this result by plotting a Poisson distribution with $\lambda = 100$ along with a Normal distribution with appropriate mean and standard deviation.

### Question 7: Visually verify that the Poisson distribution is approximately Normal for "large" $\lambda$.
...
### Question 8: What is the skewness of a Poisson distribution with $\lambda = 0.01$?
...
Plot a histogram of the sampling distribution of the standard deviation for $n=500$,  $X \sim \text{Poisson}(1)$.

### Question 9: Based on this plot, do you expect that the sampling distribution of the standard deviation approaches a normal distribution for large $n$?
...
### Question 10: Why does the CLT technically not hold using the given sampling procedure?  Explain what should we do instead if we want the CLT to guarantee convergence to Normal?
