...
His goal is to use this data to make inferences about the mean of the maximum CPU temperature achieved during a League of Legends playing session (while using his current computer and playing in the climate controlled environment).
...
Then letting $t_{n-1,.95}$ be the $95^{th}$ quantile of the $t$ distribution with $n-1$ degrees of freedom
...
\[
P(Reject) = P(\frac{\sqrt{n}}{s_n}(\bar{X_n}-\mu) + \frac{\sqrt{n}}{s_n}(\mu-x) > t_{n-1,.95}) \leq P(\frac{\sqrt{n}}{s_n}(\bar{X_n}-\mu) > t_{n-1,.95}) = ?
\]
...
So for large (and finite) $n$, the hand wavy approximation $P(Reject) \approx P(N(\frac{\sqrt{n}}{\sigma}(\mu-x),1) > z_{.95})$ is valid (where $z_{.95}$ is the $95^{th}$ quantile of the $Normal(0,1)$ distribution)
...
## Question 10: Murphy hopes that if $\mu \geq 85$, then he should reject with high probability. Using the appoximation for $P(Reject)$ given in question 9, what is the approximate rejection probability when $\mu = 85$? (Recall his planned testing procedure has $n=30$ and $x = 80$). Assume $\sigma = 10$ to answer this question.

You tell Murphy the number from question $10$ and he is thrilled. But what about if $\mu > 85$? Question 11 helps us answer this question.

## Question 11: Let $\mu_1 = 85$ and $\mu_2 > 85$. Is the approximate rejection probability under $\mu = \mu_2$ greater than, equal to, or less than the approximate rejection probability when $\mu = \mu_1$ (when $n$,$x$,$\sigma$ are given)?
