...
* Choosing 80 degrees in a little arbitrary. If the true mean of max CPU temp during playing sessions is 79.9 degrees, then under regular conditions (sample size large), the test we are about to perform should reject the null hypothesis less than 5% of the time. This isn't great; Murphy would want to know if the mean of max CPU temp during playing sessions is pretty close to 80 degrees. That would probably be enough to warrant action on his part.
...
## Question 8: Suppose instead that Murphy had studied the skewness of the population distribution, determined that $30$ was large enough to justify the use of the CLT, and that he was most concerned with accidentally concluding his computer is not overheating (i.e $\mu < 80^{\circ}$) when indeed it is. Should he use the CLT based test or the t-test?
answer: CLT based test is more powerful, and the test level (i.e probability of rejection under null hypothesis) will meet the satisfactory criteria $\leq .05$ since he somehow knows N is large enough for this.

## Question 9: Conduct the t-test and explain the results to Murphy
...
## Question 10: So if $\mu < x$, is the probability of rejecting $\leq$ or $>$ $.05$?
...
You provide him with a hand wavy argument that gives the gist:
...
## Question 11: Given the approximation $P(Reject) \approx P(N(\frac{\sqrt{n}}{\sigma}(\mu-x),1) > z_{.95})$, as $n$ gets large, for $\mu > x$, what number should $P(Reject)$ get very close to?

This explains to Murphy why choosing $x < 80$ is undesirable. He is satisfied now.
