...
For assignment 2 you collected packet data. While in the assignment you will collect much more data and analyze seconds per packet, for this lab, we'll use a much smaller version of the packet data you will collect, and we'll analyze packets per second.
...
Packets could arrive before the first timestamp of a second and after the last timestamp of a second. But we don't count them here. There may be more accurate ways to count the number of packets occurring within a fixed time interval, but lets ignore this detail for now.
