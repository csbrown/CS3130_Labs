...
We can use the logic that we used in combinations/permutations and overcounting to count up all of the different ways to get this table.  Specifically, if you recall, to get the formula for combinations, we first enumerated all the ways that we could choose a particular arrangement WITH order included, and then divided by the factor by which we overcounted.
...
There are $8 \choose 4$ ways to select $4$ of the $8$ people who `liked Cilantro`to get an `A`. That covers the enumeration of `Get A and Like Cilantro`. Now we know that if $4$ people `Get A and Like Cilantro`, then $7-4$ people must `Get A and Not Like Cilantro`, $7$ people total `Got an A`. And $4$ people in total didn't like cilantro. So $4 \choose 3$ is the number of ways to choose the remaining Cilantro haters who will `Get an A`.
...
Are `Got A` and `Likes Cilantro` independent events in our population of CS 3130 students from 2020?

## READ

Let's suppose that a student didn't have access to this two-way table. They only had access to a random sample of 

Say we want to know whether `Being a Child on the Titanic` and `Surviving the Titanic` were independent events.

We can answer this definitevly because we have access to data from all members of the titanic (i.e we had access to the entire population). The numbers `P(Being a Child on Titanic)` and `P(Surviving the Titanic)` are properties of this population.

Let's now imagine we didn't have access to the population. Instead, we have only a random sample of `100` members of the Titanic. To get this sample, run
```{r,eval = FALSE}
TitanicSample = Titanic[sample(nrow(Titanic),size = 100,replace = FALSE),]
```
